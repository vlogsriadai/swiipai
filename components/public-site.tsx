...
export function CreativeCatalogPage({ path, light, toggleTheme }: { path: string; light: boolean; toggleTheme: () => void }) {
  ...
  return <div className="public-site"><PublicHeader light={light} toggleTheme={toggleTheme} /><main className="catalog-page">
    ...
    {isModels ? <section className="section model-showcase">
      ...
      <section className="section catalog-grid">{filtered.map((tool, i) => { const {name, category: group, href, icon: Icon, description} = tool; const badge = "badge" in tool ? tool.badge : undefined; return <Link href={href} className={`catalog-card catalog-${i % 6}`} key={name}><div className="catalog-card-top"><span><Icon size={22}/></span>{badge && <b>{badge}</b>}</div><small>{group}</small><h3>{name}</h3><p>{description}</p><i>Open tool <ArrowRight size={14}/></i></Link>; })}</section>
    </>}
